...
func transSavToCheck(userID int, amount float32) {
	...
	// func (tx *Tx) QueryRow(query string, args ...interface{}) *Row
	row = tx.QueryRow("SELECT balance FROM checkings WHERE user_id = ?", userID)
	...
	fmt.Println("attempting to set checkings:", checkBalance, "savings:", savBalance)
	...
	// func (tx *Tx) Exec(query string, args ...interface{}) (Result, error)
	result, execErr = tx.Exec(`UPDATE checkings SET balance = ? WHERE user_id = ?`, checkBalance, userID)
	rowsAffected, _ = result.RowsAffected()
	fmt.Println("update savings execErr:", execErr, "rowsAffected:", rowsAffected)
	if execErr != nil || rowsAffected != 1 {
		//  func (tx *Tx) Rollback() error
